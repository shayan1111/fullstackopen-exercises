...
beforeEach(async () => {
  ...
  saltRounds = 10;
  const passwordCreationHash = await bcrypt.hash(
    passwordCreationSample,
    saltRounds,
  );
  ...
  const passwordDeletionHash = await bcrypt.hash(
    passwordDeletionSample,
    saltRounds,
  );
...
test("Make the GET request using supertest in JSON format", async () => {
  ...
  const responseToBlog = await api
    .get("/api/blogs")
    .expect(200)
...
test("Verify that making an HTTP POST request to the /api/blogs URL successfully creates a new blog post", async () => {
  ...
  const userSampleResultLogin = await api
    .post("/api/login")
...
test("Update a blog", async () => {
  ...
  await api.put(`/api/blogs/${blogToUpdateId}`).send(blogToUpdate).expect(200);

  const blogAfterUpdate = await Blog.find({});
  const updatedBlog = blogAfterUpdate.find(
...
test("Verify that if the likes property is missing from the request, it will default to the value 0", async () => {
  ...
  const sampleData = response.body.find(
    (blog) => blog.title === "A Guide to Node.js Streams",
  );
...
describe.only("When a someone creates a user,", () => {
  (test("Invalid username is yeeted", async () => {
    ...
    await api.post("/api/users").send(userSample).expect(400);

    // Make sure the size is the same before and after insertion
    const usersAfterInsertion = await User.find({});
    assert.deepStrictEqual(
      usersBeforeInsertion.length,
      usersAfterInsertion.length,
    );
